...
class UserEntity extends EntityAbstract{
	public function __structure($structure) {
		...
		$structure->indelibility = [1];
		$structure->fields = [
			'id' => [
				'#title' => lks_lang('ID'),
			],
			'name' => ['#not_listed' => true],
			'email' => [
			...
			'remember_token' => ['#not_listed' => true],
			'last_activity' => ['#not_listed' => true],
			'created_at' => ['#title' => lks_lang('Ngày tạo')],
			'updated_at' => ['#title' => lks_lang('Ngày cập nhật')],
		];
	}
	...
	public function loadEntityByEmail($email) {
		...
		return $this->loadEntityWhere([['email', '=', $email]]);
	}
}