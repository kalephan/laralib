...
abstract class EntityAbstract
{
    ...
    {
        $entity = !empty($entity_new->{$this->structure->id}) ? $this->model->find($entity_new->{$this->structure->id}) : $this->model;
    ...
    public function loadEntityWhere($where)
    {
        ...
        if (!$cache) {
            ...
            $this->_modelAddWhere($model, $where);
    ...
    private function _modelAddWhere($model, $where) {
    ...
    private function _modelAddOrdeyBy($model, $order = []) {
        if (count($this->structure->order_by)) {
            $order = array_merge($order, $this->structure->order_by);
        }
    ...
    private function _modelPaginate($model, $items_per_page = null)
    {
        $items_per_page = $items_per_page ? $items_per_page : config('lks.items_per_page', 20);

        $paginator = $model->paginate($items_per_page);
    ...
    {
        $model = $this->model->select($this->structure->id);
        $this->_modelAddOrdeyBy($model);

        return $this->_modelPaginate($model, $items_per_page);
    }

    public function loadEntityWherePaginate($where, $items_per_page = null)
    {
        $model = $this->model->select($this->structure->id);
        $this->_modelAddWhere($model, $where);
        $this->_modelAddOrdeyBy($model);
