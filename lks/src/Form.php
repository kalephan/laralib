...
class Form {
	public function build($class, $form_values = []) {
		...
		if ($cache_value = Cache::get($cache_name_error_form)) {
			...
			if ($cache_value = Cache::get($cache_name)) {
				...
				call_user_func([new $class[0], $class[1]], $form);
	...
	public static function buildItem($item, &$error = []) {
		...
		if (!empty($item['#options_callback'])) {
			...
			$item['#options'] = call_user_func_array([$lks->load($options_callback[0]), $options_callback[1]], $item['#options_callback']['arguments']);
		}
	...
	public static function render($key, &$form) {
		...
		if (count($item) && isset($item['#type'])) {
			...
			return view($template, array('element' => $item));
		}
	...
	public static function submit() {
		...
		if (!empty($form->validate)) {
			foreach ($form->validate as $value) {
				...
				if (! call_user_func_array([new $segment[0], $segment[1]], $data)) {
					$validate = false;
				}
			}
		}
		...
		if ($validate) {
			if (!empty($form->submit)) {
				foreach ($form->submit as $value) {
					...
					call_user_func_array([new $segment[0], $segment[1]], $data);
				}
			}
