...
class TopicEvent {
    ...
    public function alterCreateForm($form_id, &$form) {
        ...
        unset($form['province_id']['#title'],
              $form['shipping']['#title'],
              $form['coupon_type']['#title'],
              $form['content']['#title']);
        ...
        $product_form['coupon_value']['#attributes'] = ['placeholder' => lks_lang('Khuyến mãi')];
        unset($product_form['coupon_type']['#title']);
        unset($product_form['coupon_start']['#title']);
        $product_form['coupon_start']['#attributes'] = ['placeholder' => lks_lang('Thời điểm bắt đầu')];
        $product_form['coupon_start']['#attributes']['class'] .= 'datepicker datepicker_start';
        unset($product_form['coupon_end']['#title']);
        $product_form['coupon_end']['#attributes'] = ['placeholder' => lks_lang('Thời điểm kết thúc')];
    ...
    public function alterCreateFormValue($form_id, &$form, &$form_values) {
        ...
        if ($parents) {
            foreach ($parents as $cat) {
                $form_values['category_text'] .= lks_anchor("s?c3=" . $cat->id, $cat->title, ['target' => '_blank']);
            }
        }

        $lks->asset->jsAdd(['topic_create_datepicker_group' => $form['#settings']['product_items']], 'settings');
    }
