...
class ShopEvent
{
    ...
    public function alterCreateForm($form_id, &$form)
    {
        ...
        $form['#group_shop']['accepted'] = array(
            ...
            '#title' => lks_lang('Tôi đã đọc và đồng ý với những :term', array(
                ':term' => lks_anchor('article/8', lks_lang('quy định và điều khoản sử dụng shop'), array(
                    'target' => '_blank'
                ))
            )),
            '#validate' => 'accepted',
    ...
    public function alterUpdateFormValidate($form_id, &$form, &$form_values)
    {
        ...
        if (! empty($form_values['shop_website']) && substr($form_values['shop_website'], 0, 7) != 'http://' && substr($form_values['shop_website'], 0, 8) != 'https://') {
