...
class UserActivationController {
    ...
    public function showUserActivationResendFormSubmit($form_id, $form, &$form_values) {
        ...
        lks_mail($form_values['email'],
            lks_lang(config('lks.create user activation resend email subject', 'Gửi lại mã kích hoạt tài khoản của bạn')),
            lks_render('user-create-activation-resend-email', $vars)
        );
    }
}