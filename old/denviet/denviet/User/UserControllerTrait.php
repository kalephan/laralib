...
trait UserControllerTrait
{
    ...
    function showLoginFormValidate($form_id, &$form, &$form_values)
    {
        ...
        if (! isset($user->email)) {
            $form->error['email'][] = $error_msg . lks_lang('Tài khoản !email không tồn tại.', [
    ...
    function showForgotPasswordFormSubmit($form_id, $form, &$form_values)
    {
        ...
        lks_mail($form_values['email'], lks_lang(config('lks.user forgotpass email subject', 'Thiết lập lại mật khẩu của bạn')), lks_render('user-reset-password-email', $vars));
    }
