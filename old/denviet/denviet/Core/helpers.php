...
if (!function_exists('lks_entity_contextual_link_get')) {
    function lks_entity_contextual_link_get($entity, $structure) {
        ...
        if ($lks->response->isAdminPanel()
            && !empty($structure['#action_links']['read'])
        ...
        if ($lks->response->isAdminPanel()
            && !empty($structure['#action_links']['preview'])
            && (isset($entity->active) && $entity->active != 1)
        ) {
        ...
        if (!empty($structure['#action_links']['active'])
             && (isset($entity->active) && $entity->active != 1)
        ) {
        ...
        if (!empty($structure['#action_links']['approve'])
             && (isset($entity->approve) && $entity->approve)
        ) {
...
function lks_style($path, $style = 'normal', $attributes = []) {
    ...
    elseif ($img_site
        && substr($path, 0, strlen($img_site)) == $img_site
        && config('lks.cdn image style make', 0)
    ) {
