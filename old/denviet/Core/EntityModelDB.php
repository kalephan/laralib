...
class EntityModelDB
{
    ...
    {
        $attributes['select'] = isset($attributes['select']) ? $attributes['select'] : [
            $structure->id
        ];
    ...
    public static function buildLoadEntityWhere(&$db, $structure, $attributes = [])
    {
        ...
        if (isset($lks->request)) {
            ...
            if ((! isset($attributes['filter']) || $attributes['filter'] == true) && is_array($filter) && count($filter)) {
    ...
    {
        $db = DB::table($structure->table)->where($structure->id, '=', $entity->{$structure->id})->update(lks_object_to_array($entity));
